...
class GenericCollection<T extends Comparable<T> & IHasTimestamp> {
    private final Map<String, Set<T>> elementsByCategory;


    public GenericCollection() {
        this.elementsByCategory = new HashMap<>();
    }

    public void addGenericItem(String category, T element) {
        elementsByCategory.computeIfAbsent(category, k -> new TreeSet<>(Comparator.reverseOrder())).add(element);
    }
    ...
    public Collection<T> itemsFromCategories(List<String> categories) {
        return categories.stream()
                .flatMap(c -> elementsByCategory.get(c).stream())
                .collect(Collectors.toCollection(() -> new TreeSet<T>(Comparator.reverseOrder())));
    }

    public Map<String, Set<T>> byMonthAndDay() {
        Map<String, Set<T>> result = new TreeMap<>();
//        elementsByCategory.values().stream()
//                .flatMap(Collection::stream)
//                .forEach(e -> {
//                    String key = String.format("%02d-%02d", e.getTimestamp().getMonthValue(), e.getTimestamp().getDayOfMonth());
//                    result.putIfAbsent(key, new TreeSet<>(Comparator.reverseOrder()));
//                    result.get(key).add(e);
//                });
//        return result;
        return elementsByCategory.values().stream()
                .flatMap(Collection::stream)
                .collect(Collectors.groupingBy(e ->
                                String.format("%02d-%02d", e.getTimestamp().getMonthValue(), e.getTimestamp().getDayOfMonth())
