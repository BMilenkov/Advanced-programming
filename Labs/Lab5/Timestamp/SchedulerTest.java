...
class Timestamp<T> implements Comparable<Timestamp<T>> {
    ...
    public String toString() {
        return String.format(time + " " + element);
    }
}
...
class Scheduler<T> {
    ...
    public Timestamp<T> next() {
        ...
        return timestamps.stream()
                .filter(time -> time.getTime().isBefore(now))
    ...
    public Timestamp<T> last() {
        LocalDateTime now = LocalDateTime.now();
        return timestamps.stream()
                .filter(time -> time.getTime().isAfter(now))
    ...
    public List<Timestamp<T>> getAll(LocalDateTime begin, LocalDateTime end) {
        ...
        return timestamps.stream()
                .filter(tTimestamp -> tTimestamp.getTime().isAfter(begin)
                        && tTimestamp.getTime().isBefore(end))
